...
.PHONY: rootfs root app app_bak
rootfs:root app app_bak
root:
	...
	mkdir -p $(OUTPUTDIR)/customer_bak
#	mkdir -p $(OUTPUTDIR)/rootfs/kmsglog
#	mkdir -p $(OUTPUTDIR)/kmsglog
	if [ "$(BOARD)" = "010A" ]; then \
	...
	mkdir -p $(OUTPUTDIR)/appconfigs/source
...
#	echo 'echo csc 0 3 50 50 40 50 0 0 > /proc/mi_modules/mi_disp/mi_disp0' >> $(OUTPUTDIR)/rootfs/etc/profile
	...
	if [ -f "$(KBUILD_ROOT)/$(FLASH_TYPE)/modules/kernel_mod_list_late" ]; then \
		...
		echo -e "insmod /customer/res/mdrv_crypto.ko\ninsmod /customer/res/cryptodev.ko\n# kernel_mod_list\nexport LD_LIBRARY_PATH=\$$LD_LIBRARY_PATH:/config/lib:/customer/lib:/customer_app/lib\nsleep 2\ncp /appconfigs/wpa_supplicant.conf /etc/\n/customer/wpa_supplicant -Dnl80211 -iwlan0 -c /appconfigs/wpa_supplicant.conf &\n">> $(LATE_INIT_FILE); \
	fi;
	##sleep 1\nudhcpc -i wlan0 -s /etc/init.d/udhcpc.script &
	#-----------------------kernel ko end.-------------------#
	
	echo mkdir /var/lock >> $(OUTPUTDIR)/rootfs/etc/profile
	...
	#----------mount customer start----------#
	echo -e "if [ \`/etc/fw_printenv -n use_bak_rootfs\` -eq 1 ]; then" >> $(OUTPUTDIR)/rootfs/etc/profile
	echo -e "	mount -t ubifs ubi0:customer_bak /customer" >> $(OUTPUTDIR)/rootfs/etc/profile
	echo -e	"else" >> $(OUTPUTDIR)/rootfs/etc/profile
	echo -e "	mount -t ubifs ubi0:customer /customer" >> $(OUTPUTDIR)/rootfs/etc/profile
	echo -e	"fi" >> $(OUTPUTDIR)/rootfs/etc/profile
	#-----------mount customer end---------------#
	echo export LD_LIBRARY_PATH=\$$LD_LIBRARY_PATH:\/config\/lib:/customer_app/lib:/customer/lib >> $(OUTPUTDIR)/rootfs/etc/profile;

	echo "busybox telnetd&" >> $(OUTPUTDIR)/rootfs/etc/profile
	#echo \/customer\/bin\/zkgui \& >> $(OUTPUTDIR)/rootfs/etc/profile;
	echo sleep 1 >> $(OUTPUTDIR)/rootfs/etc/profile;
...
#	echo "fi;" >> ${OUTPUTDIR}/rootfs/etc/profile
	...
	echo -e "echo csc 0 3 50 50 40 50 0 0 > /proc/mi_modules/mi_disp/mi_disp0" >> $(OUTPUTDIR)/rootfs/etc/profile;	
	...
	mkdir -p $(OUTPUTDIR)/media
	mkdir -p $(OUTPUTDIR)/rootfs/vendor
	mkdir -p $(OUTPUTDIR)/rootfs/customer
	mkdir -p $(OUTPUTDIR)/rootfs/media
	mkdir -p $(OUTPUTDIR)/ota
	mkdir -p $(OUTPUTDIR)/rootfs/ota
	mkdir -p $(OUTPUTDIR)/kmsglog
	mkdir -p $(OUTPUTDIR)/rootfs/kmsglog
#        mkdir -p $(OUTPUTDIR)/kmsglog	
	# ------ rootfs_bak start ------ #
	mkdir -p  $(OUTPUTDIR)/rootfs_bak
	cp -rvfP $(OUTPUTDIR)/rootfs/*  $(rootfs_bak$(RESOUCE))

	cp -rvfP $(OUTPUTDIR)/customer/demo.sh $(OUTPUTDIR)/customer_bak/demo.sh
