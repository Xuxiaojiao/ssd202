...
%_$(FLASH_TYPE)_ramfs_script:
	...
	@echo nand erase.part $(patsubst %_$(FLASH_TYPE)_ramfs_script,%,$@) >> $(SCRIPTDIR)/[[$(patsubst %_$(FLASH_TYPE)_ramfs_script,%,$@).es
	@echo nand write.e $(TFTPDOWNLOADADDR) $(patsubst %_$(FLASH_TYPE)_ramfs_script,%,$@) \$${filesize} >> $(SCRIPTDIR)/[[$(patsubst %_$(FLASH_TYPE)_ramfs_script,%,$@).es
	@echo setenv initrd_high ${INITRAMFSLOADADDR} >> $(SCRIPTDIR)/[[$(patsubst %_$(FLASH_TYPE)_ramfs_script,%,$@).es
	@echo setenv initrd_size \$${filesize} >> $(SCRIPTDIR)/[[$(patsubst %_$(FLASH_TYPE)_ramfs_script,%,$@).es
...
ubi_$(FLASH_TYPE)_config_script:
	...
	@echo setenv ota_upgrade_status 1 >> $(SCRIPTDIR)/set_config
	@echo setenv use_bak_rootfs 0 >> $(SCRIPTDIR)/set_config
