...
void autoboot_command(const char *s)
{
	...
	run_command("bootlogo 0 0 0 0 0" , 0);
	mdelay(100);
	run_command("gpio out 4 1" , 0);
	run_command("gpio out 5 1" , 0);

        //for rootfs bak
       	printf("132\n");
        env = getenv("use_bak_rootfs");
        if(!strcmp(env, "1"))
        {
			size = get_partition_size("rootfs_bak");
            printf("load ramdisk from rootfs_bak, part_size=0x%x\n",size);
            char  cmd_str[128];
            sprintf(cmd_str, "nand read.e 0x21000000 rootfs_bak 0x%x",size);
            run_command(cmd_str, 0);
        }
        else
        {
			size = get_partition_size("rootfs");
            printf("load ramdisk from rootfs, part_size=0x%x\n",size);
            char  cmd_str[128];
            sprintf(cmd_str, "nand read.e 0x21000000 rootfs 0x%x",size);
            run_command(cmd_str, 0);
        }
