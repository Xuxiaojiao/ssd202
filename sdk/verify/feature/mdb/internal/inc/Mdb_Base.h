...
#define RUN_COMMAND(func, out, _fmt, _args...) do{ \
        char cmdString[128];    \
        std::vector<std::string> strVect;   \
        snprintf(cmdString, 128, _fmt, ## _args); \
        printf("Run cmd:%s->%s\n", #func, cmdString);  \
        Mdb_Base::ParseStrings(cmdString, strVect); \
        func(strVect, out, 0); \
    }while(0);
...
class Mdb_Base
{
    ...
    public:
        ...
        typedef void (Mdb_Base::*SUBCMD_FUNC)(std::vector<std::string> &, std::string &, int);
                            ...
                            if (*itVectStr == strMdbBaseInStrings[0])
                            {
                                for (itVectCmds = mapPushedData[*itVectStr].begin(); itVectCmds != mapPushedData[*itVectStr].end(); ++itVectCmds)
                                {
                                    ...
                                    if (it != mapMdbBaseCmd.end())
                                    {
                                        ...
                                        {
                                            (this->*(it->second.fpFunc))(itVectCmds->strInStrings, ignoreStr, 0);
                                        }
                                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (it != mapMdbBaseCmd.end())
                    {
                        ...
                        {
                            (this->*(it->second.fpFunc))(strMdbBaseInStrings, strOut, 0);
                        }
                        else if (strMdbBaseInStrings.size() == 1 && (strMdbBaseInStrings[0] == "help" || strMdbBaseInStrings[0] == "h"))
                        {
                            (this->*(it->second.fpFunc))(strMdbBaseInStrings, strOut, 1);
                        }
                        else
                        {
                            Print(strOut, "Cmd Para error!\n", PRINT_COLOR_YELLOW, PRINT_MODE_HIGHTLIGHT);
